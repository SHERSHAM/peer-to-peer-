    ...
    (async () => {
      ...
      if (profiles) {
        const enriched = await Promise.all(profiles.map(async p => {
          const { count: marks } = await supabase.from("student_marks")
            .select("*", { count: "exact", head: true })
            .eq("student_id", p.id);
          return { ...p, activityCount: marks || 0 };
        }));
        setUsers(enriched);
      }
      setLoading(false);
  ...
  return (
    <div style={{ minHeight:"100vh", position:"relative", zIndex:1 }}>
      ...
      <div style={{ maxWidth:1200, margin:"0 auto", padding:"32px 20px" }}>
        ...
        <SectionLabel color="#00f5ff">Connected Users ({users.length})</SectionLabel>

        {loading ? (
          <div style={{ textAlign:"center", padding:60, color:"#506880" }}>Loading users…</div>
        ) : users.length === 0 ? (
          <div style={{ textAlign:"center", padding:60, color:"#506880", fontFamily:"'Share Tech Mono',monospace" }}>No users yet.</div>
        ) : (
          <div style={{ overflowX:"auto" }}>
            <table style={{ width:"100%", borderCollapse:"collapse", fontFamily:"'Share Tech Mono',monospace", fontSize:12 }}>
              <thead>
                <tr style={{ borderBottom:"2px solid rgba(0,245,255,.3)" }}>
                  <th style={{ textAlign:"left", padding:"14px", color:"#00f5ff", fontWeight:700 }}>NAME</th>
                  <th style={{ textAlign:"left", padding:"14px", color:"#00f5ff", fontWeight:700 }}>ROLE</th>
                  <th style={{ textAlign:"left", padding:"14px", color:"#00f5ff", fontWeight:700 }}>EMAIL</th>
                  <th style={{ textAlign:"center", padding:"14px", color:"#00f5ff", fontWeight:700 }}>ACTIVITY</th>
                  <th style={{ textAlign:"left", padding:"14px", color:"#00f5ff", fontWeight:700 }}>LAST ACTIVE</th>
                </tr>
              </thead>
              <tbody>
                {users.map((u, i) => {
                  const roleColor = { admin:"#bf00ff", sir:"#ffee00", student:"#00f5ff" }[u.role] || "#506880";
                  const lastActive = u.last_active ? new Date(u.last_active).toLocaleDateString() : "—";
                  return (
                    <tr key={u.id} style={{ borderBottom:"1px solid rgba(0,245,255,.1)", background:i%2==="0"?"transparent":"rgba(0,245,255,.02)" }}>
                      <td style={{ padding:"14px", color:"#dff0ff" }}>{u.name || "—"}</td>
                      <td style={{ padding:"14px" }}>
                        <span style={{ background:`${roleColor}18`, border:`1px solid ${roleColor}44`, color:roleColor, padding:"4px 10px", borderRadius:12, fontSize:10, fontWeight:700, letterSpacing:1, textTransform:"uppercase" }}>
                          {u.role}
                        </span>
                      </td>
                      <td style={{ padding:"14px", color:"#506880", fontSize:11 }}>{u.email || "—"}</td>
                      <td style={{ padding:"14px", textAlign:"center", color:"#00ff88", fontWeight:700 }}>{u.activityCount}</td>
                      <td style={{ padding:"14px", color:"#506880" }}>{lastActive}</td>
                    </tr>
                  );
                })}
              </tbody>
    ...
    <div style={{ minHeight:"100vh", position:"relative", zIndex:1 }}>
      ...
      <div style={{ maxWidth:1100, margin:"0 auto", padding:"32px 20px" }}>
        ...
        <div style={{ marginBottom:28 }}>
          <Btn color="#bf00ff" onClick={onUsers} style={{ marginRight:12 }}>👥 View Users</Btn>
        </div>
...
export default function App() {
  ...
  function goUsers()    { setScreen("users"); }
  function backToDash() { setScreen("dashboard"); }
